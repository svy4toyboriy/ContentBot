...
Бот развернут и запущен на облачном сервере от Selectel. Также написан небольшой скрипт - `chech.sh` - для отслеживания объема скачанных за все время файлов, которые удаляет наиболее старые файлы, чтобы не происходило переполнение диска. 
...
1)	Получение от пользователя команды /start, после чего бот сообщает о том, что он готов к работе. Далее пользователь выбирает язык и формат контента. По умолчанию выбран - аудио. Его всегда можно легко сменить соответствующей командой /video и обратно /audio.
